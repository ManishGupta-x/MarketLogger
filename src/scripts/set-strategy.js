...
function printUsage() {
  console.log(`
Usage: node set-strategy.js --grid=<value> --target=<value> --sl=<value> --per-trade=<value> --capital=<value> [--force]

Parameters:
  --grid        Grid percentage for buying (e.g., 0.25 for 0.25% drop to buy)
  --target      Target percentage for selling (e.g., 0.5 for 0.5% rise to sell)
...
Example:
  node set-strategy.js --grid=0.25 --target=0.5 --sl=1 --per-trade=5000 --capital=100000
`);
}
...
async function setStrategy() {
  ...
  const targetPercentage = parseFloat(params['target']);
  ...
  if (isNaN(gridPercentage) || isNaN(targetPercentage) || isNaN(stopLossPercentage) || isNaN(perTradeAmount) || isNaN(capital)) {
    console.error('Error: All parameters must be valid numbers');
    process.exit(1);
  }

  if (gridPercentage <= 0 || targetPercentage <= 0 || stopLossPercentage <= 0 || perTradeAmount <= 0 || capital <= 0) {
  ...
  console.log(`Grid (Buy): ${gridPercentage}%`);
  console.log(`Target (Sell): ${targetPercentage}%`);
